...
async function main() {
    ...
    try {
        ...
        const [joinRequestId, wrappedB64] = qrData.split('|');
        console.log("Join Request ID:", joinRequestId);
        console.log("Wrapped AES Key (Base64):", wrappedB64);

        const aesKey = await decrypt_private_key(wrappedB64);

        saveAESKey(roomId, aesKey);

        // Call the API to accept the join request
        const approveResponse = await voteOnJoinRequest(roomId, joinRequestId, true);
        console.log("Approve join request response:", approveResponse.status);

        // Retry if approval failed
        if (approveResponse.status !== 200) {
            console.log("Approval failed: ", approveResponse);
            throw new Error("Approval of join request failed");
        } else {
